...
from ethics_filter import EthicsMatcher
...
def main() -> None:
    ...
    parser.add_argument(
        "--ethics-title-only",
        action="store_true",
        help="Only match ethics when it appears in the course title.",
    )
    ...
    ethics_matcher = EthicsMatcher.build(include_description=not args.ethics_title_only)
    ...
    unique_courses = (
        ...
        .merge(ethics_flags, on=[args.prefix_col, args.number_col], how="left")
    )
    ...
    ethics_subset = unique_courses[unique_courses["is_ethics_related"]].sort_values(
        [args.prefix_col, args.number_col]
    )

    full_output = output_dir / "nau_courses_with_ai_flag.csv"
    ai_output = output_dir / "nau_courses_ai_subset.csv"
    ethics_output = output_dir / "nau_courses_ethics_subset.csv"
    ...
    ethics_subset.to_csv(ethics_output, index=False)
    ...
    print(f"Ethics-only subset: {ethics_output}")
